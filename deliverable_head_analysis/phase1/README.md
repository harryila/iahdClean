...
- **Random seed:** 42 (ensures reproducibility - same split every run)
- **Split method:** Manual shuffle-based split using Python's `random` module
  - Originally planned to use `sklearn.model_selection.train_test_split()`, but encountered a numpy version conflict (sklearn compiled against numpy 1.x, we have numpy 2.x)
  - Manual implementation produces equivalent results
...
- ✅ **Sufficient samples:** Each question type has enough samples in both splits for meaningful analysis
...
Loads `test_samples.json` to evaluate the causal importance of identified heads. Tests whether ablating heads affects accuracy on unseen samples.
