...
Tests causal importance of heads identified by the Wu24 Retrieval Head method
...
- Replicates Figure 8 from Wu24 paper

The heads are loaded dynamically from Phase 2 Wu24 results.

Ablation Mechanism:
- Uses forward pre-hooks on o_proj to zero out head outputs before projection
- Same mechanism as Method 1, but with heads from Wu24 (Method 2 Phase 2) results

Reference: WU_Retrieval_Head/needle_in_haystack_with_mask.py (Wu24 ablation logic)
"""
import argparse
import json
import os
...
PHASE2_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "phase2", "retrieval_head_wu24", "results")
...
def load_top_heads_from_phase2(model_key, question_key, total_tokens, top_n=50):
    """
    Load top N heads from Phase 2 Wu24 results.
...
def main():
    parser = argparse.ArgumentParser(description="Phase 3 Method 2: Wu24 Ablation Study")
    ...
    print("=" * 70)
    print(f"Phase 3 Method 2 (Wu24) Ablation: {model_key} / {question_key} / {total_tokens} tokens")
    ...
    # Load top heads from Phase 2 Wu24 results
    ...
    print(f"Loaded {len(top_heads_full)} top heads from Phase 2 Wu24 results")
    ...
    results = {
        ...
        "heads_source": "phase2_wu24_results",
