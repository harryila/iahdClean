...
Tests causal importance of heads identified by the QRHead method
...
- Replicates Figure 8 format from Wu24 paper

The heads are loaded dynamically from Phase 2 QRHead results.

Ablation Mechanism:
- Uses forward pre-hooks on o_proj to zero out head outputs before projection
- Same mechanism as Methods 1 and 2 (consistent across all Phase 3 methods)

Note: QRHead originally uses heads for retrieval SCORING, not ablation.
We apply ablation to test if the identified heads are CAUSALLY important.
"""
import argparse
import json
import os
...
PHASE2_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "phase2", "qrhead", "results")
...
class HeadAblator:
    ...
    This is the same mechanism used across all Phase 3 methods for consistency.
    It is most similar to Wu24's Flash Attention ablation (zeroing query states).
    """
...
def load_top_heads_from_phase2(model_key, question_key, total_tokens, top_n=50):
    """
    Load top N heads from Phase 2 QRHead results.
...
def main():
    parser = argparse.ArgumentParser(description="Phase 3 Method 3: QRHead Ablation Study")
    ...
    print("=" * 70)
    print(f"Phase 3 Method 3 (QRHead) Ablation: {model_key} / {question_key} / {total_tokens} tokens")
    ...
    # Load top heads from Phase 2 QRHead results
    ...
    print(f"Loaded {len(top_heads_full)} top heads from Phase 2 QRHead results")
    ...
    results = {
        ...
        "heads_source": "phase2_qrhead_results",
